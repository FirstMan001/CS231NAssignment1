...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    loss = np.sum(-np.log(probs[range(N), y])) / N + reg * np.sum(W1 * W1) + reg * np.sum(W2 * W2)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    #############################################################################
    dprobs = probs
    dprobs[xrange(probs.shape[0]), y] -= 1
    dprobs /= N
    dW2 = relu.T.dot(dprobs) + reg*W2*2
    grads['W2'] = dW2
    db2 = np.ones([dprobs.shape[0]]).dot(dprobs)
    grads['b2'] = db2
    dmid = np.matmul(dprobs, W2.T)
    dmid[relu<=0] = 0
    dW1 = X.T.dot(dmid) + reg*W1*2 
    grads['W1'] = dW1
    db1 = np.ones([dmid.shape[0]]).dot(dmid)
    grads['b1'] = db1
  ...
  def predict(self, X):
    ...
    score = np.matmul(relu, self.params['W2'])+self.params['b2']
